...
class NMMONet(nn.Module):
    ...
    def __init__(self, observation_space, num_actions, use_lstm=False):
        ...
        self.fc = nn.Linear(384*2 + 256 + 64, 512)
    ...
    def forward(self, input_dict, h = None, training=False):
        ...
        terrain = input_dict["terrain"]                             #[T, B, 8, 15, 15]
        camp = input_dict["camp"]                                   #[T, B, 8, 15, 15]
        entity = input_dict["entity"]                               #[T, B, 7, 15, 15]
        player_info = input_dict["player_info"]                     #[T, B, 8, 26]
        cur_task = input_dict["cur_task"][:,::8,:].reshape(-1, 5)     #[T, B, 5]
        global_terrain = input_dict["global_terrain"][:,::8,:,:].reshape(-1, 160, 160)               #[TXB, 160, 160]
        global_camp = input_dict["global_camp"][:,::8,:,:].reshape(-1, 160, 160)                     #[TXB, 160, 160]
        ...
        #     print("cur_task", cur_task.shape)
        #     print("global_terrain", global_terrain.shape)
        #     print("global_camp", global_camp.shape)
        #     print("input_dict[global_terrain]", input_dict["global_terrain"][:,::8,:,:].shape)
        #     print("input_dict[global_camp]", input_dict["global_camp"][:,::8,:,:].shape)

        terrain = F.one_hot(terrain, num_classes=6).permute(0, 1, 4, 2, 3)                          #[T, B, 6, 15, 15]
        camp = F.one_hot(camp, num_classes=4).permute(0, 1, 4, 2, 3)[:,:,1:4,:,:]                   #[T, B, 3, 15, 15]
        global_terrain = F.one_hot(global_terrain, num_classes=6).permute(0, 3, 1, 2)            #[TXB, 6, 160, 160]
        global_camp = F.one_hot(global_camp, num_classes=4).permute(0, 3, 1, 2)[:,1:4,:,:]     #[TXB, 3, 160, 160]
        ...
        x = torch.cat([terrain, camp, entity], dim=2)                                               #[T, B, 16, 15, 15]
        global_x = torch.cat([global_terrain, global_camp], dim=1).to(torch.float)                                  #[TXB, 9, 15, 15]
        ...
        #     print("test4")
        #     print("x", x.shape)
        #     print("player_info_x", player_info_x.shape)
        ...
        #     print("global_x", global_x.shape)
        #     print("player_info_x", player_info_x.shape)
        #     print("cur_task_x", cur_task.shape)


        x = torch.cat([x, player_info_x], dim=-1).reshape(-1, 8, 384)                 #[T, B, 384]
        ...
        global_x =  torch.cat([global_x, cur_task_x], dim=1).unsqueeze(1).repeat(1, 8, 1)    #[T, B, 320]

        # if training:
        #     print("test6")
        #     print("x", x.shape)
        #     print("global_x", global_x.shape)
        #     print("atten_output", atten_output.shape)

        #print(x.shape, atten_output.shape, global_x.shape)
        x = torch.cat([x, atten_output, global_x], dim=-1).reshape(T * B, -1)      #[T, B, 384 + 384 + 320]

        #x, h = self.gru(x, h)
        x = F.relu(self.fc(x))
        x = F.relu(self.final_fc(x))
